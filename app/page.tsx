...
export default async function Home() {
  ...
  const userId = user?.id || PERSONAL_USER_ID
  ...
  if (!profile) {
    ...
    const { data: newProfile, error } = await supabase
      ...
      .insert({
        id: userId,
        username: user?.email?.split("@")[0] || "Player One",
  ...
  // Fetch quests (global + user's custom)
  ...
  if (!quests || quests.length === 0) {
    // Seed global quests
    const globalQuests = demoQuests.map(q => ({ ...q, is_global: true, user_id: null }))
    const { data: newQuests } = await supabase.from("quests").insert(globalQuests).select().order("sort_order")
